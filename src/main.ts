...
app.once("ready", async () => {
	try {
		const result = await dialog.showOpenDialog({
			title: "Choose song folder to scan for .ksh files",
			properties: ["openDirectory", "multiSelections"],
		})

		let charts = 0

		for (const songFolder of result.filePaths) {
			console.info(`Searching ${songFolder}`)
			for await (const songFile of glob("**/*.ksh", { cwd: songFolder })) {
				console.info(`Reading ${songFile}`)

				const content = await readFile(join(songFolder, songFile), "utf8")
